/**
 * Created by kmsharjas on 09/02/2022.
 */
 ...
 export class SearchService implements PipeTransform {
   ...
   private filterByObject(filter) {
     return value => {
       for (const key in filter) {
         if (key === '$or') {
           if (!this.filterByOr(filter.$or)(SearchService.getValue(value))) {
             return false;
           }
           continue;
         }

         if (!value || !SearchService.isFoundOnWalking(value, key)) {
           return false;
         }

         if (!this.isMatching(filter[key], SearchService.getValue(value[key]))) {
           return false;
         }
       }
   ...
   transform(array: any[], filter: any): any {
     ...
     switch (typeof filter) {
       ...
       case 'string':
         if (SearchService.isNumber(filter)) {
